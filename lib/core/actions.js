...
const addPageAndRouteAction = async (name, dest) => {
  ...
  //2.写入文件
  let targetDest = path.resolve(dest, name.toLowerCase());
  console.log(name, dest, targetDest, "-------------");
...
const addStoreAction = async (name, dest) => {
  //1.编译
  const storeReult = await compile("vue-store.ejs", {});
  ...
  if (createDirSync(targetDest)) {
    ...
    writeToFile(targetStorePath, storeReult);
